...
`,
    testCases: [
      ...
      {
        id: '3',
        description: 'Attention weights shape is (seq_len, seq_len)',
        input: 'scaled_dot_product_attention(np.random.randn(6, 16), np.random.randn(6, 16), np.random.randn(6, 16))[1].shape',
        expected: '(6, 6)',
        hidden: true,
      },
      {
        id: '4',
        description: 'Output is finite (no NaN or inf)',
        input: '(lambda: [np.random.seed(0), (out := scaled_dot_product_attention(np.random.randn(4, 8), np.random.randn(4, 8), np.random.randn(4, 8))[0]), bool(np.all(np.isfinite(out)))][-1])()',
        expected: 'True',
        hidden: true,
      },
      {
        id: '5',
        description: 'Attention weights are non-negative (valid probabilities)',
        input: '(lambda: [np.random.seed(1), (weights := scaled_dot_product_attention(np.random.randn(5, 10), np.random.randn(5, 10), np.random.randn(5, 10))[1]), bool(np.all(weights >= 0))][-1])()',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '2',
        description: 'Different d_model size (32 dims, 4 heads)',
        input: 'multi_head_attention(np.random.randn(6, 32), np.random.randn(6, 32), np.random.randn(6, 32), 4).shape',
        expected: '(6, 32)',
        hidden: true,
      },
      {
        id: '3',
        description: 'Single head case (equivalent to standard attention)',
        input: 'multi_head_attention(np.random.randn(4, 16), np.random.randn(4, 16), np.random.randn(4, 16), 1).shape',
        expected: '(4, 16)',
        hidden: true,
      },
      {
        id: '4',
        description: 'Output is finite (no NaN or inf)',
        input: '(lambda: [np.random.seed(42), (out := multi_head_attention(np.random.randn(4, 64), np.random.randn(4, 64), np.random.randn(4, 64), 8)), bool(np.all(np.isfinite(out)))][-1])()',
        expected: 'True',
        hidden: true,
      },
      {
        id: '5',
        description: 'Large d_model with many heads (128 dims, 16 heads)',
        input: 'multi_head_attention(np.random.randn(3, 128), np.random.randn(3, 128), np.random.randn(3, 128), 16).shape',
        expected: '(3, 128)',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '3',
        description: 'First position odd dim starts with cos(0)=1',
        input: 'bool(np.isclose(positional_encoding(10, 4)[0, 1], 1.0))',
        expected: 'True',
        hidden: true,
      },
      {
        id: '4',
        description: 'Even columns use sin, odd columns use cos pattern',
        input: '(lambda: (pe := positional_encoding(100, 8), bool(np.allclose(pe[:, 0], np.sin(np.arange(100) / (10000.0 ** (0.0 / 8))))))[-1])()',
        expected: 'True',
        hidden: true,
      },
      {
        id: '5',
        description: 'Values are bounded between -1 and 1',
        input: '(lambda: (pe := positional_encoding(50, 64), bool(np.all(pe >= -1.0) and np.all(pe <= 1.0)))[-1])()',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '3',
        description: 'Variance approximately 1 after normalization (gamma=1, beta=0)',
        input: 'bool(np.allclose(layer_norm(np.random.randn(5, 64), np.ones(64), np.zeros(64)).var(axis=-1), 1.0, atol=0.05))',
        expected: 'True',
        hidden: true,
      },
      {
        id: '4',
        description: 'Works with 3D input (batch, seq_len, features)',
        input: 'bool(np.allclose(layer_norm(np.random.randn(2, 3, 8), np.ones(8), np.zeros(8)).mean(axis=-1), 0.0, atol=1e-5))',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '3',
        description: 'Diagonal is all zeros (each position attends to itself)',
        input: 'bool(np.all(np.diag(create_causal_mask(5)) == 0.0))',
        expected: 'True',
        hidden: true,
      },
      {
        id: '4',
        description: 'Upper triangle is all -inf',
        input: '(lambda: (mask := create_causal_mask(4), upper := mask[np.triu_indices(4, k=1)], bool(np.all(upper == float("-inf"))))[-1])()',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '4',
        description: 'Both Q_rot and K_rot returned with correct shapes',
        input: '(lambda: (result := apply_rope(np.ones((5, 16)), np.ones((5, 16))), bool(result[0].shape == (5, 16) and result[1].shape == (5, 16)))[-1])()',
        expected: 'True',
        hidden: true,
      },
      {
        id: '5',
        description: 'K is also rotated (not identical to input)',
        input: '(lambda: (K := np.ones((4, 8)), K_rot := apply_rope(K, K)[1], bool(not np.allclose(K_rot[1:], K[1:])))[-1])()',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      },
      {
        id: '4',
        description: 'Output is finite (no NaN or inf)',
        input: '(lambda: [np.random.seed(7), (out := grouped_query_attention(np.random.randn(4, 64), np.random.randn(4, 16), np.random.randn(4, 16), 8, 2)), bool(np.all(np.isfinite(out)))][-1])()',
        expected: 'True',
        hidden: true,
      },
    ],
...
`,
    testCases: [
      ...
      {
        id: '5',
        description: 'Diagonal is always allowed (self-attention)',
        input: 'bool(np.all(np.diag(sliding_window_mask(6, 1)) == 0.0))',
        expected: 'True',
        hidden: true,
      },
    ],
